...
// FORK: locale plugin — auth config error copy translates via the plugin tree.
    ...
    <div className="card w-full max-w-2xl bg-base-100 border border-base-300 shadow-xl">
      <div className="card-body gap-4">
        <h2 className="card-title gap-2">
          ...
          {t("Authentication setup required")}
        </h2>
        ...
        {isHostedMode ? (
          <p className="text-sm text-base-content/70">
            ...
            {t(", and Google OAuth credentials on the deployment.")}
          </p>
        ) : (
          <p className="text-sm text-base-content/70">
            {t("Cloudflare Access mode requires")}{" "}
            <code className="mx-1">TEAM_DOMAIN</code>{" "}
            {t("(a full https URL) and")}{" "}
