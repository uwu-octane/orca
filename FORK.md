...
| 2026-08-28 | Cordis plugin layer, phase 4: Intl threading — 9 client files format via `getIntlLocale(readActiveLocale())` instead of hardcoded `"en-US"` (rank tracking, search performance, audit, AI search, billing charts); server `ga4DateInTimeZone` takes an optional locale (default `"en-US"`, additive).                                                                             | (phase 4) |
