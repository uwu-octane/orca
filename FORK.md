...
| 2026-08-28 | Cordis plugin layer, fix 2: the boot-settle re-render from `1954961` didn't reach consumers — React skips context propagation when the value is Object.is-equal, and `ctx` is stable, so hooks mounted during the boot window stayed frozen on the English fallback (language switcher had no visible effect). The `settled` flag now lives in the context value itself, flipping its identity so consumers re-subscribe to the real services.      | (fix 2)   |
